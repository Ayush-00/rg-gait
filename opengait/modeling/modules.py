...
class Occ_Detector_Amount(torch.nn.Module):

    def __init__(self, parts_num):
        super(Occ_Detector_Amount, self).__init__()
    ...
    def forward(self, x, seqL):
        ...
        if seqL is not None:
            ...
            out = out.view(b, f, -1)
            out = out.squeeze(0) # (f, 64)
            occ_amts = self.layer_amount(out)    # (f,1)
            ...
            for i,l in enumerate(seqL[0]):
                output_lis.append(torch.mean(out[accumulator:accumulator+l], dim=0))
                amt_lis.append(torch.mean(occ_amts[accumulator:accumulator+l], dim=0))
                accumulator += l
            ...
            out = out.view(b, f, -1)
            out = torch.mean(out, dim=1) # (n,c=64)
            occ_amts = self.layer_amount(out).squeeze()    # (n)
            occ_embed = out.unsqueeze(2).repeat(1,1,self.parts_num)     #(n,c,p)
            
            return occ_embed, occ_amts

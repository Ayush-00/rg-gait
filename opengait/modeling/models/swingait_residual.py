...
from ..modules import HorizontalPoolingPyramid, PackSequenceWrapper, SeparateFCs, SeparateBNNecks, SetBlockWrapper, ParallelBN1d, Occ_Detector_Amount
...
class SwinGait_Component(nn.Module):
    def __init__(self, model_cfg):
        super(SwinGait_Component, self).__init__()
...
class SwinGait_Residual(BaseModel):
    def __init__(self, cfgs, training): 
        self.T_max_iter = cfgs['trainer_cfg']['T_max_iter']
        super(SwinGait_Residual, self).__init__(cfgs, training=training)

    def build_network(self, model_cfg):
        self.full_component = SwinGait_Component(model_cfg)
        self.mimic_component = SwinGait_Component(model_cfg)
        self.BNNecks = SeparateBNNecks(**model_cfg['SeparateBNNecks'])
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        transformer_params = list(self.mimic_component.transformer.named_parameters())
        params_list = [
            ...
            {'params': self.mimic_component.FCs.parameters(), 'lr': optimizer_cfg['lr'] * 0.1, 'weight_decay': optimizer_cfg['weight_decay']}, 
    ...
    def init_parameters(self):
        ...
        if 'mimic_cfg' in self.cfgs['model_cfg']:
            ...
            if 'init_mimic' in self.cfgs['model_cfg']['mimic_cfg']:
                raise NotImplementedError('Mimic initialization not implemented yet for swingait_residual')
            ...
            raise NotImplementedError('Mimic model not specified in the configuration file')
    ...
    def forward(self, inputs):
        ...
        embed = embed.contiguous()[:, :, :-1]  # [n, p, c]
        embed_2 = embed_2.contiguous()[:, :, :-1]  # [n, p, c]
        logits = logits.contiguous()[:, :, :-1]  # [n, p, c]

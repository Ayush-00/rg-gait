...
class BaseModel(MetaModel, nn.Module):
    ...
    def run_test(model):
        ...
        if 'repeat' in model.cfgs['evaluator_cfg'].keys():
            num_repeats = model.cfgs['evaluator_cfg']['repeat']
            
            eval_results = []
            
            for rep in range(num_repeats):
                if rank == 0:
                    print(f"\nStarting evaluation {rep+1}/{num_repeats}....\n")
                with torch.no_grad():
                ...
                if rank == 0:
                    ...
                    eval_results.append(eval_func(info_dict, dataset_name, **valid_args))
                    
            if torch.distributed.get_rank() == 0:       
                save_name = f"{model.cfgs['evaluator_cfg']['save_name']}_repeat_{num_repeats}.pkl"
